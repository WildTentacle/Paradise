		...
		switch(href_list["implant"])
			...
			if("ertadd")
				...
				if(src in SSticker.mode.cult)
					...
					if(istype(cult))
						cult.memorize_cult_objectives(src)
					to_chat(current, "<span class='warning'><FONT size = 3><B>The nanobots in the ert mindshield implant remove all thoughts about being in a cult.  Have a productive day!</B></FONT></span>")
					memory = ""
